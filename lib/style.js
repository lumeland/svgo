import { csstree, specificity, stable } from "../deps.js";
import { matches, visit } from "./xast.js";
import { compareSpecificity } from "./css-tools.js";
import {
  attrsGroups,
  inheritableAttrs,
...
export function collectStylesheet(root) {
  const stylesheet = [];
  ...
  visit(root, {
    element: {
      enter: (node) => {
        if (node.name === "style") {
          const dynamic = node.attributes.media != null &&
...
export function computeStyle(stylesheet, node) {
