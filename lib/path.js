...
function readNumber(string, cursor) {
  let i = cursor;
  let value = "";
...
export function parsePathData(string) {
  const pathData = [];
  let command = null;
...
function stringifyNumber({ number, precision }) {
  let result;
  if (precision == null) {
...
export function stringifyPathData(
  { pathData, precision, disableSpaceAfterFlags },
) {
